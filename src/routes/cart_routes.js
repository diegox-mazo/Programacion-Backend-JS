...
router.post("/:cart_id/products/:product_id", async (request,response)=>{
    try {
        ...
        let result = await cart_manager.updateProductInCart(cart_id, product_id);
...
//  api/carts/[cart_id]/products/[product_id]    (DELETE)
router.delete("/:cart_id/products/:product_id", async (request,response)=>{
    try {
        /* let cart_id = Number(request.params.cart_id);
        let product_id = Number(request.params.product_id);
        ...
        let cart_id = request.params.cart_id;
        let product_id = request.params.product_id;
        let result = await cart_manager.deleteProductInCart(cart_id, product_id);
